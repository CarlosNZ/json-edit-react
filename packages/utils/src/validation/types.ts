// A node location, mirroring core's `NodeData['path']`: an array of object keys
// and array indices. `CollectionKey` isn't part of core's public API, so the
// element type is inlined.
type Path = (string | number)[]

/**
 * One normalised validation problem. An adapter such as `ajvAdapter` maps a
 * validation library's native error into this shape, or a hand-written
 * normaliser can produce it directly.
 */
...
/**
 * A normalised validator: runs over the whole document and returns the current
 ...
 * is O(1), and the object's *reference* is stable while the error set is
 * unchanged — see `useValidationState` for why that matters.
 */
export interface ValidationState {
  /** True when there are no issues. */
  isValid: boolean
  /** Every issue, in the order the normaliser produced them. */
