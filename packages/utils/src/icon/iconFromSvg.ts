...
// is greedy so it reaches the final </svg>, and it's case-insensitive for
// <SVG>. Anything before the tag (<?xml?>, <!DOCTYPE>, comments) is ignored:
// this locates only the root tag, never the inner markup.
...
// (core sizes via `scale` and would override it anyway), the xml namespaces
// (React adds them), and id/class/style (theming and identity are core's, and a
...
// stroke-width → strokeWidth, fill-rule → fillRule, … so a lifted root
// attribute is a valid React SVG prop. The inner markup is NOT converted: it's
// injected raw, where kebab-case is correct.
...
// markup rides through verbatim via dangerouslySetInnerHTML, which the browser
// parses at mount in the SVG namespace (the <g> sits inside core's <svg>), so
// nested or odd markup works. dangerouslySetInnerHTML is benign here: the
...
// Props read straight off a React <svg> element: already camelCase, no parsing.
// `viewBox`/`children` are handled separately, and the rest are size and
// namespace attributes core owns.
...
// so core doesn't wrap a second <svg> around it; any other element (a <path>,
// fragment, custom component) becomes the glyph content directly.
 ...
 *
 * The element and object forms are NOT interned, an inline element being a
 * fresh object each render, so hoist or memoise them for render stability.
 */
