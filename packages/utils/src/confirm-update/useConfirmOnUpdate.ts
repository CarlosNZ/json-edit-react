...
/**
 * The declarative wrapper, built on {@link useJsonEditorConfirm}. For the
 * common case — "ask before these events" — it pre-writes the gate: it calls
 * `control.hold()` synchronously, so the editor stays open and the tree is
 * blocked while the dialog is up, awaits the consumer's modal, then
 * `release()`s on confirm, or returns `null` (a silent cancel, with nothing
 * applied) on dismiss. The consumer wires the returned `dialog` to their modal.
 *
 * Core commits optimistically by default, so gating an edit on a dialog means
 * opting out via `hold()`, which MUST be called synchronously, before the
 * first `await`. That timing, and bridging the imperative "ask" to a
 * render-driven modal via {@link useJsonEditorConfirm}, is what this packages
 * up.
 *
 * The returned `onUpdate`'s identity changes when options are passed inline,
 * but core reads `onUpdate` through a ref, so that churn is harmless and
 * consumers needn't memoise their options.
 */
  ...
  const onUpdate: UpdateFunction<T> = useCallback(
    async (input, control) => {
      ...
      if (release) {
        ...
        // Dismissal is a silent cancel: the gate never released, so nothing
        // was applied and the editor closes with the original value.
        if (!ok) return null
        // Confirmation applies the value and closes the editor, leaving any
        // `inner` work below to settle in the background.
        release()
      }
