...
The one exception is `react-datepicker`'s own stylesheet in the widgets entry: the library is external, so that import survives as an `import` statement for the consumer's bundler and there's no text to inline. It's the one place `sideEffects: false` misdescribes the package, and the one stylesheet that isn't per-component — the widgets entry also exports `ReactSelect` and `CodeEditor`, so a consumer of either still loads react-datepicker's CSS (~24 kB raw / ~3.3 kB gzip). Accepted rather than deferred into the lazy chunk: our overrides in `ReactDatePicker/style.css` beat the library's rules only by cascade order at equal specificity, and loading the library's sheet after our `useStyles` injection would silently invert that. Per-widget sub-paths are the real fix if a consumer ever reports it.
