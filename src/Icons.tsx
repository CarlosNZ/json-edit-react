...
// Shared <svg> wrapper for all icons. `size` maps to width/height, and the most
// common attributes (24×24 viewBox, fill="currentColor") are defaulted here so
// each icon only needs to declare the attributes that differ. Any other SVG
// attribute (stroke, transform, baseProfile, viewBox overrides, …) passes
// through unchanged.
const IconSvg: React.FC<{ size: string } & React.SVGProps<SVGSVGElement>> = ({
  size,
  viewBox = '0 0 24 24',
  fill = 'currentColor',
  children,
...
export const IconAdd: React.FC<IconProps> = (props): JSX.Element => (
...
export const IconEdit: React.FC<IconProps> = (props): JSX.Element => (
  // icon:bx-edit | Boxicons https://boxicons.com/ | Atisa
  <IconSvg {...props} transform="translate(0, 0.5)">
...
export const IconDelete: React.FC<IconProps> = (props): JSX.Element => (
...
const sharedSVGProps = {
  ...
  strokeLinejoin: 'round' as 'round' | 'inherit' | 'bevel' | 'miter' | undefined,
  strokeWidth: 2,
}

export const IconCopy: React.FC<IconProps> = (props): JSX.Element => (
  // icon:clipboard-copy | Lucide https://lucide.dev/
  <IconSvg {...props} {...sharedSVGProps}>
...
export const IconOk: React.FC<IconProps> = (props): JSX.Element => (
...
export const IconCancel: React.FC<IconProps> = (props): JSX.Element => (
  // icon:cancel | Typicons https://www.s-ings.com/typicons/ | Stephen
  // Hutchings
  <IconSvg {...props} baseProfile="tiny">
...
export const IconChevron: React.FC<IconProps> = (props): JSX.Element => (
  // icon:chevron-down | FontAwesome https://fontawesome.com/
  <IconSvg {...props} viewBox="0 0 512 512">
...
export const Icon: React.FC<IconSharedProps> = ({ name, nodeData }): JSX.Element => {
  ...
  switch (name) {
    ...
    case 'delete':
      return (
        icons?.delete ?? (
          <IconDelete {...commonProps} style={getStyles('iconDelete', nodeData)} size="1.45em" />
        )
      )
    case 'copy':
      return (
        icons?.copy ?? (
          <IconCopy {...commonProps} style={getStyles('iconCopy', nodeData)} size="1.2em" />
        )
      )
    case 'ok':
      return (
        icons?.ok ?? (
          <IconOk {...commonProps} style={{ fontSize: '90%', ...getStyles('iconOk', nodeData) }} />
        )
      )
    case 'cancel':
      return (
