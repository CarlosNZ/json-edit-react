...
- §6 New `UpdateFunction` return shape ✅ (shipped under §17 Category 2) — [#249](https://github.com/CarlosNZ/json-edit-react/issues/249) (§7 per-node `isValid` dropped)
- §8 `restrict*` → `allow*` rename ✅ — [#250](https://github.com/CarlosNZ/json-edit-react/issues/250) / [#318](https://github.com/CarlosNZ/json-edit-react/pull/318)
- §9 Group the prop surface — dropped (referential-stability/bundle cost; §17 converged on flat `allow*`. Display-prop consistency renames rode along with §8 — see §9 below.)
- §10 `useImperativeHandle` triggers ✅ — [#251](https://github.com/CarlosNZ/json-edit-react/issues/251)
- §12 `onRenameProperty` callback ✅ (shipped under §17 as `rename` event on `onUpdate` / `onEditEvent`) — [#252](https://github.com/CarlosNZ/json-edit-react/issues/252)
...
- §17 API standardisation — the three-category model ✅ (umbrella over §6/§8/§10/§12; `onEventIntercept` parked, all other categories shipped) — [#289](https://github.com/CarlosNZ/json-edit-react/issues/289)
...
Follow-up: extend the handle to **key / add** *session* modes — [#286](https://github.com/CarlosNZ/json-edit-react/issues/286). Implemented under §17 Phase 4 (Category 4) as `startRename` / `startAdd` session openers sharing one `confirm`/`cancel`, **then deferred to 2.x in the Tier 1 trim (see the Update below)** — the shipped 2.0 handle is `collapse` + `startEdit` + `confirm` + `cancel`. The *direct-mutation* half of #286 was declined (see §17 Category 4): a consumer owns `data`/`setData`, so a data-mutating command is redundant. The #117 confirmation flow is `onEventIntercept` gate → consumer's own modal → consumer mutates its own data (no `editorRef.delete`).

**Update (Tier 1, post-[#294](https://github.com/CarlosNZ/json-edit-react/issues/294) bundle review):** `startRename` / `startAdd` and the awaitable `confirm()` were **deferred to 2.x** — they were the largest removable slice of the §17 bundle growth (forensics in [v2-api-bundle-analysis.md](dev-docs/v2-api-bundle-analysis.md)). The shipped 2.0 handle is `collapse` + `startEdit` (now returning `true | 'RESTRICTED' | 'PATH_NOT_FOUND'`, the minimal restricted-vs-not-found signal) + a click-based void `confirm()` + `cancel()`. The rename/add session *events* still fire via `onEditEvent` for UI-driven sessions; only the imperative openers are gone, to return with #286 in 2.x.
...
> **Decided at implementation (Phase 4, [#299](https://github.com/CarlosNZ/json-edit-react/issues/299)): the direct mutators were DROPPED.** The Phase 4 `JsonEditorHandle` was **UI-interactions only** — session openers (`startEdit`/`startRename`/`startAdd`), shared `confirm`/`cancel`, and `collapse`. (`startRename`/`startAdd` were then deferred in the Tier 1 trim — see the Update below; the *shipped* handle is `collapse` + `startEdit` + `confirm` + `cancel`.) The direct mutators (`delete`/`edit`/`add`/`rename`/`move`) sketched below were cut: a consumer owns `data`/`setData`, so mutating data is already `setData(newData)` and the editor reflects it — a command for it is a redundant second path (and the binary-`CommandResult`-can't-report-the-new-path tradeoff noted below dissolves). This also decouples Phase 4 from [#117](https://github.com/CarlosNZ/json-edit-react/issues/117): the confirm-before-delete *resume* becomes "consumer intercepts → runs its modal → deletes from its own data", needing no `editorRef.delete`. #117 closes when `onEventIntercept` lands; [#286](https://github.com/CarlosNZ/json-edit-react/issues/286) (extending the handle to mutation modes) is largely mooted — its session-opener half shipped here, its direct-mutation half is declined by design.
>
> **Update (Tier 1, post-[#294](https://github.com/CarlosNZ/json-edit-react/issues/294) bundle review): `startRename`/`startAdd` and the awaitable `confirm()`/`CommandResult` were further deferred to 2.x.** They were the biggest removable slice of the §17 bundle growth ([v2-api-bundle-analysis.md](dev-docs/v2-api-bundle-analysis.md)). The shipped 2.0 handle is `collapse` + `startEdit` (→ `true | 'RESTRICTED' | 'PATH_NOT_FOUND'`) + a click-based void `confirm()` + `cancel()`; the session-commit registry that backed the awaitable `confirm()` is removed. The rename/add session *events* still fire (`onEditEvent`) for UI-driven sessions.
>
...
Verdict: **a 2.x decision, gated on the bundle-test repo above — not a 2.0 scramble.** The payoff (~1 kB) is small relative to the cost: a new public-API seam (hard to walk back) plus threading a context-resolved adapter through the two components [PERF-ARCHITECTURE.md](dev-docs/PERF-ARCHITECTURE.md) warns about (referential stability of every node prop). Get the real per-feature number from actual bundlers first; pursue only if DnD shows up as a meaningful slice and users ask. If it stays ~1 kB, leave it bundled — the one-import "it just works" feel is worth more than the kilobyte.
