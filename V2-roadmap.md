...
>
> **Update (Tier 1, post-[#294](https://github.com/CarlosNZ/json-edit-react/issues/294) bundle review): `startRename`/`startAdd` and the awaitable `confirm()`/`CommandResult` were further deferred to 2.x.** They were the biggest removable slice of the §17 bundle growth. The shipped 2.0 handle is `collapse` + `startEdit` (→ `true | 'RESTRICTED' | 'PATH_NOT_FOUND'`) + a click-based void `confirm()` + `cancel()`; the session-commit registry that backed the awaitable `confirm()` is removed. The rename/add session *events* still fire (`onEditEvent`) for UI-driven sessions.
>
