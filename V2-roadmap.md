...
**Update (Tier 1, post-[#294](https://github.com/CarlosNZ/json-edit-react/issues/294) bundle review):** `startRename` / `startAdd` and the awaitable `confirm()` were **deferred to 2.x** — they were the largest removable slice of the §17 bundle growth (forensics in [v2-api-bundle-analysis.md](v2-api-bundle-analysis.md)). The shipped 2.0 handle is `collapse` + `startEdit` (now returning `true | 'RESTRICTED' | 'PATH_NOT_FOUND'`, the minimal restricted-vs-not-found signal) + a click-based void `confirm()` + `cancel()`. The rename/add session *events* still fire via `onEditEvent` for UI-driven sessions; only the imperative openers are gone, to return with #286 in 2.x.
...
>
> **Update (Tier 1, post-[#294](https://github.com/CarlosNZ/json-edit-react/issues/294) bundle review): `startRename`/`startAdd` and the awaitable `confirm()`/`CommandResult` were further deferred to 2.x.** They were the biggest removable slice of the §17 bundle growth ([v2-api-bundle-analysis.md](v2-api-bundle-analysis.md)). The shipped 2.0 handle is `collapse` + `startEdit` (→ `true | 'RESTRICTED' | 'PATH_NOT_FOUND'`) + a click-based void `confirm()` + `cancel()`; the session-commit registry that backed the awaitable `confirm()` is removed. The rename/add session *events* still fire (`onEditEvent`) for UI-driven sessions.
>
