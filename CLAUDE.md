...
- [dev-docs/](dev-docs/) — internal development docs: not shipped, not user-facing. The toolchain + release reference ([dev-docs/package-management-guide.md](dev-docs/package-management-guide.md) — quick-reference playbooks on top, detailed reference below) plus its condensed per-package companion ([dev-docs/publishing-cheat-sheet.md](dev-docs/publishing-cheat-sheet.md)), performance architecture + measurement ([dev-docs/PERF-ARCHITECTURE.md](dev-docs/PERF-ARCHITECTURE.md), [dev-docs/PERF-MEASUREMENT.md](dev-docs/PERF-MEASUREMENT.md) for the manual in-browser procedure, [dev-docs/PERF-BENCH.md](dev-docs/PERF-BENCH.md) for the headless `pnpm bench` suite), and editing-model design notes ([dev-docs/EditingModel-new.md](dev-docs/EditingModel-new.md), temporary — fold in and delete when the model lands). Keep these current as the code evolves.
...
| [style.css](src/style.css) + [injectStyles.ts](src/injectStyles.ts) | Bundled CSS (inlined as a minified string by the `inlineCss` rollup plugin in [scripts/rollup-inline-css.mjs](scripts/rollup-inline-css.mjs)) and the injector that puts it in `<head>` |
...
- **Component stylesheets are imported as strings (`./style.css?inline`) and injected from a `useStyles` call in the component**, the same shape as core's [injectStyles.ts](src/injectStyles.ts). A bare `import './style.css'` is dropped from the build outright — `@rollup/plugin-node-resolve` reads the package's own `sideEffects: false` and shakes it out — which is how the package published with none of its CSS (issue #398). Injecting per component also keeps each stylesheet reachable only from the component that renders it, so the #388 per-component shaking still means something. Verify CSS changes with `pnpm demo:pack`: `local` mode runs the same `useStyles` path (Vite serves `?inline` as text), so it confirms the rules but can't catch the CSS going missing from the build — only the packed artefact can.
...
- Only `build/` ships per package. Core's `tsconfig.json` deliberately sets `"files": ["src/index.ts", "src/css.d.ts"]` — the entry, plus the ambient declaration for the `?inline` stylesheet import, which has to be a root file since there's no `include` glob — so rollup's TS plugin walks the dependency graph from the entry rather than typechecking the whole tree at build time. Use `pnpm compile` for a full project-wide typecheck.
...
- **Every package ships each entry as a single bundled file, so `sideEffects: false` can't tree-shake them** — it's module-granular, and there's only one module. What makes unused exports droppable is the `/*#__PURE__*/` annotations each rollup config stamps onto eager top-level factory calls (`React.memo`, `createContext`, `lazy`, `createDefinitionFactory`, core's `mergeIcons`, and utils' `intern` / `internRef` / `internRefs`), plus terser's `format.preserve_annotations`. Core additionally annotates the compiled `jsx` calls in [defaultTheme.tsx](src/contexts/ThemeProvider/defaultTheme.tsx), whose JSX is top-level *data* rather than a render body. All four packages have an esbuild-based `verify-treeshake.mjs` wired into their `build` script as the end-to-end guard — esbuild deliberately, since rollup infers local purity on its own and would mask a regression. Renaming any annotated internal function without updating the config's `pureNames` list silently regresses the bundle; the guard is what catches it. Background in issues #382 (themes), #388 (components), #389 (core), #406 (utils).
- **Sub-packages typecheck against core's `build/`, not its source.** Each resolves `json-edit-react` through a symlink to the repo root, whose `types` field points at the gitignored `build/index.d.ts` — so a stale or missing core build produces phantom errors *in the sub-package* (`UpdateFunctionProps is not generic`, `no exported member 'IconDefinition'`), all pointing at the wrong package. Rebuild core first. `pnpm -r build` and `pnpm pack-all` both order core ahead of the sub-packages, so those are safe; a lone `pnpm compile` / `pnpm build` / `pub:<sub-package>` in one sub-package is not. The errors don't stop a build either: `@rollup/plugin-typescript` only escalates a diagnostic to fatal when `noEmitOnError` is set, and no package sets it, so the build emits its `.d.ts` anyway and the errors scroll past as warnings.
- **Core's stylesheet is a plain string constant, not a top-level injection** — no annotation can drop a bare side-effecting statement, so the `inlineCss` plugin ([scripts/rollup-inline-css.mjs](scripts/rollup-inline-css.mjs), shared with components) emits the CSS as `export default '<minified css>'` with no injector call, and [injectStyles.ts](src/injectStyles.ts) injects it from a `useInsertionEffect` in [JsonEditor.tsx](src/JsonEditor.tsx). That keeps the CSS reachable only from the editor path (so a helper-only import shakes it out) while guaranteeing the rules are in `<head>` before any layout effect runs or the browser can paint. `--jer-form-border`, a custom property that appears only in the stylesheet, is a `verify-treeshake.mjs` marker because of this. The import specifier is `./style.css?inline` — Vite serves a plain `.css` import as an injected side effect with no default export, which breaks any direct consumer of `src/` (the demo's `local` mode); rollup has no query convention, so `inlineCss` resolves the specifier itself (keeping the `?inline` marker on the module id so its `load` hook handles exactly these imports), minifies the file with esbuild, and jest's `moduleNameMapper` matches the query too. One upshot: `pnpm dev` exercises the same injection path as the published bundle. Issue #396.
