...
- **Two dist-tag rules.** Core betas always publish with `--tag beta` (baked into `pub:core:beta`) so core's stable `latest` (`1.30.1`) never moves; only the explicit `pub:core:latest` touches `latest`. Sub-packages have no stable release, so each beta *is* their `latest` — they publish with no `--tag`. Also: core's `version` must be bumped before packing any sub-package, since each freezes `"json-edit-react": "workspace:^"` into `^<core's current version>` at pack time.
