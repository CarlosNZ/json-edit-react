...
- **All three packages ship as a single bundled file, so `sideEffects: false` can't tree-shake them** — it's module-granular, and there's only one module. What makes unused exports droppable is the `/*#__PURE__*/` annotations each rollup config stamps onto eager top-level factory calls (`React.memo`, `createContext`, `lazy`, `createDefinitionFactory`, and core's `mergeIcons`), plus terser's `format.preserve_annotations`. Core additionally annotates the compiled `jsx` calls in [defaultTheme.tsx](src/contexts/ThemeProvider/defaultTheme.tsx), whose JSX is top-level *data* rather than a render body. Core, themes and components each have an esbuild-based `verify-treeshake.mjs` wired into their `build` script as the end-to-end guard — esbuild deliberately, since rollup infers local purity on its own and would mask a regression. Renaming any annotated internal function without updating the config's `pureNames` list silently regresses the bundle; the guard is what catches it. Background in issues #382 (themes), #388 (components), #389 (core).
