...
**Why:** one prop doing two unrelated jobs (a boolean toggle *and* a callback) was awkward to type and document. The split matches the wider v2 callback model — enablement is a gate, the copy notification is an observer — and `onCopy` now receives the same flat [`NodeData`](https://carlosnz.github.io/json-edit-react/) payload every other callback gets.
