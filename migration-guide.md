...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| React peer dependency raised to **≥18** (was ≥16)                                                                                                                                                                                                                                                       | Upgrade React and `react-dom` to 18+ if you're still on 16 / 17 — see [React 18 or newer required](#react-18-or-newer-required)                                                                                                                  |
...
| Misc public-export changes — new `AutogrowTextArea`; `toPathString` is now `/`-encoded; `ThemeStyles` is `Partial`; the stylesheet is injected on first mount, not at import                                                                                                                                                                                      | Mostly additive; act only if you parse `toPathString` output, typed against a total `ThemeStyles`, render `StringDisplay` / `StringEdit` with no editor mounted, or override `.jer-*` rules from runtime-injected CSS — see [Misc changes to public exports](#14-misc-changes-to-public-exports)                                                                   |
...
## React 18 or newer required

The React peer dependency is now `react >=18.0.0` — v1 allowed `>=16.0.0`. If your app is still on React 16 or 17, upgrade React (and `react-dom`) to 18 or newer before installing v2; there's no further migration work beyond the upgrade itself.

**Why:** v2's fine-grained re-rendering is built on `useSyncExternalStore`, a hook added in React 18, so 16 / 17 can no longer be supported. The same `react >=18.0.0` peer dependency applies to the companion packages (`@json-edit-react/themes`, `@json-edit-react/components`, `@json-edit-react/utils`).
...
The bundled stylesheet is injected into `<head>` when the first editor or viewer mounts, rather than at import time. For an app that renders the editor there's still nothing to import, and the rules are in place before the editor's first paint. The exception is rendering the exported building blocks — `StringDisplay`, `StringEdit`, `AutogrowTextArea` — **on their own**, with no editor mounted anywhere on the page: those now render unstyled unless you import the stylesheet yourself with `import 'json-edit-react/style.css'`.

The timing also affects one narrow cascade case. The `<style>` element still lands at the **end** of `<head>`, so nothing changes relative to stylesheets that are in the document before any JavaScript runs — a `<link>` or `<style>` in the HTML, or CSS extracted into your main bundle. But CSS that reaches `<head>` *at runtime* — CSS-in-JS, a dev server's style injection, or the extracted CSS of a lazy-loaded route — now lands before the editor's stylesheet whenever it arrives before the first editor mounts, so such a rule overriding a `.jer-*` rule at **equal specificity** stops winning. Make the selector more specific and source order stops mattering:
