...
- `ReactSelect` — a `react-select` wrapper that satisfies the `CustomSelectProps` contract. Pass to `JsonEditor`'s `CustomSelect` prop to replace the built-in native `<select>` used for type changes, enum value picking, and the new-key dropdown.
