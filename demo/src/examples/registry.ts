...
const allExamples: Record<string, ExampleDef> = {
  ...
  'edit-restrictions': {
    ...
    blurb:
      "Every per-node permission that governs **editing** in one place to play with — drag-and-drop has its own page. `allowEdit`, `allowDelete` and `allowAdd` are each a filter function run per node.\n\nTwo boxes differ only in `allowAdd`: **addable** shows a ✚ and accepts new properties; **sealed** takes none. Each holds the same four leaves, labelled by the two powers a node controls — editing its value and deleting it: **🟢** edit + delete, **🟡** delete only, **🟠** edit only, **🔴** neither.\n\nRenaming a key has no permission of its own — it's a delete of the old key plus an add of the new one *to its box*. So a key is renamable only when the node is **deletable** and its box is **addable**: in *addable*, 🟢 and 🟡 rename while 🟠 and 🔴 stay put; in *sealed*, nothing renames. Note **🟡** — its value is frozen, yet its key still renames, because a rename never consults `allowEdit`.\n\nA third box, **editable**, is itself editable as raw JSON (`allowEdit` on the collection). Rewriting it that way changes all its contents at once — overriding every per-leaf rule inside, so even a 🔴 leaf can be changed. A per-node value restriction only ever governs editing that node *in place*; if its collection can be edited as JSON, the restriction is just a speed bump.",
